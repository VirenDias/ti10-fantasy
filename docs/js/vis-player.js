d3.csv("./resources/ti10_fantasy_points.csv")
  .then(function(rawData) {
    ...
    const svg = d3.select("#vis-player")
      .select("svg")

    const chartGroup = d3.select("#vis-player")
      .select("svg")
      .select("g")
    ...
    var renderChart = function(filterPosition, filterOutcome, sort) {
      ...
      const relevantData = d3.rollup(filteredData, g => d3.mean(g, d => d.total), d => d.player_name)
    ...
    d3.select("#vis-player .btn-filter-position")
      .selectAll("button")
      .on("click", function() {
        d3.select("#vis-player .btn-filter-position").selectAll("button").classed("active", false);
    ...
    d3.select("#vis-player .btn-filter-outcome")
      .selectAll("button")
      .on("click", function() {
        d3.select("#vis-player .btn-filter-outcome").selectAll("button").classed("active", false);
        d3.select(this).classed("active", true);
        renderChart(
          filterPosition = d3.select("#vis-player .btn-filter-position .active").attr("d"),
          filterOutcome = d3.select("#vis-player .btn-filter-outcome .active").attr("d"),
    ...
    d3.select("#vis-player .btn-sort")
      .selectAll("button")
      .on("click", function() {
        d3.select("#vis-player .btn-sort").selectAll("button").classed("active", false);
        d3.select(this).classed("active", true);
        renderChart(
          filterPosition = d3.select("#vis-player .btn-filter-position .active").attr("d"),
          filterOutcome = d3.select("#vis-player .btn-filter-outcome .active").attr("d"),
