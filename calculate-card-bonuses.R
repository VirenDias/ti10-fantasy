...
fantasy_summary <- read.csv(
  "data/fantasy_summary_pre_ti.csv", 
  colClasses = c(player_id = "character")
)
...
# TI10 Fantasy Group Stage Day 1
playing_teams <- c(
  "Alliance", 
  "Evil Geniuses",
  "INVICTUS GAMING", 
...
card_summary %>% 
  filter(team_name %in% playing_teams, role == "Core") %>% 
  arrange(-total)
card_summary %>% 
  filter(team_name %in% playing_teams, role == "Mid") %>% 
  arrange(-total)
card_summary %>% 
  filter(team_name %in% playing_teams, role == "Support") %>% 
  arrange(-total)
